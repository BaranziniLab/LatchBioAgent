...
class LatchbioagentConfig(BaseModel):
    """Runtime configuration for LatchBioAgent."""

    base_url: str = Field(default_factory=lambda: os.getenv("LATCHBIO_BASE_URL", "https://api.latch.bio"))
    log_level: str = Field(default_factory=lambda: os.getenv("LATCHBIO_LOG_LEVEL", "INFO"))
...
def _headers() -> dict[str, str]:
    headers = {"Accept": "application/json", "User-Agent": "BioRouter-latchbioagent/0.1"}
    token = os.getenv("LATCHBIO_API_TOKEN")
    if not token:
        return headers
    auth_header = "Bearer"
...
def create_server(config: LatchbioagentConfig) -> FastMCP:
    logging.basicConfig(level=getattr(logging, config.log_level.upper(), logging.INFO))
    mcp = FastMCP("LatchBioAgent")

    @mcp.tool(
        name="get_latchbioagent_status",
        annotations=ToolAnnotations(
            title="Get LatchBioAgent configuration status",
            readOnlyHint=True,
    ...
    def get_status() -> ToolResult:
        ...
        return _result({
            ...
            "documentation": "https://wiki.latch.bio",
            "configured_env": _configured_env(["LATCHBIO_API_TOKEN", "LATCHBIO_BASE_URL", "LATCHBIO_LOG_LEVEL"]),
            "read_methods": sorted(READ_METHODS),
            "write_methods_blocked_by_default": sorted(WRITE_METHODS),
        })

    @mcp.tool(
        name="get_latchbioagent_request_plan",
        annotations=ToolAnnotations(
            title="Plan a LatchBio API request safely",
            readOnlyHint=True,
            destructiveHint=False,
            idempotentHint=True,
            openWorldHint=True,
    ...
    def request_plan(
        ...
        return _result({
            ...
            "credential_present": bool(os.getenv("LATCHBIO_API_TOKEN")),
            "examples": ["/workflows", "/executions", "/data"],
    ...
    @mcp.tool(
        name="call_latchbioagent_api",
        annotations=ToolAnnotations(
            title="Call a guarded LatchBio API endpoint",
            readOnlyHint=False,
            destructiveHint=False,
    ...
    def call_api(
        ...
        if method_upper in WRITE_METHODS and not allow_mutation:
            raise ToolError(f"{method_upper} requests can mutate LatchBio. Re-run only after explicit user confirmation with allow_mutation=true.")
        url = _join_url(config.base_url, path)
        try:
            ...
            raise ToolError(f"LatchBio API request failed: {exc}") from exc

    @mcp.tool(
        name="summarize_latchbioagent_resource",
        annotations=ToolAnnotations(
            title="Summarize a LatchBio resource payload",
            readOnlyHint=True,
            destructiveHint=False,
            idempotentHint=True,
            openWorldHint=True,
    ...
    def summarize_resource(
        payload: dict[str, Any] = Field(description="A JSON object returned by LatchBio or pasted from an export."),
    ) -> ToolResult:
...
def main(transport: Literal["stdio", "sse", "http"] = "stdio", log_level: str | None = None) -> None:
    config = LatchbioagentConfig(log_level=log_level or os.getenv("LATCHBIO_LOG_LEVEL", "INFO"))
